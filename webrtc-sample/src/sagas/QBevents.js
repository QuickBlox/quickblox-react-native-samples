...
function* handleCallEvent(event) {
  const {session: incomingSession} = event.payload;
  ...
  if (
    session &&
    session.id.toLowerCase() !== incomingSession.id.toLowerCase()
  ) {
    const username = user ? user.fullName || user.login || user.email : 'User';
    yield put(
      webrtcReject({
...
function createWebRTCChannel() {
  return eventChannel(emitter => {
    ...
    const QBWebRTCEvents = Object.keys(QB.webrtc.EVENT_TYPE).map(
      key => QB.webrtc.EVENT_TYPE[key],
    );
...
export default [spawn(QBconnectionEventsSaga), spawn(QBWebRTCEventsSaga)];
